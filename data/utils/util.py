...
def generate_mask_based_on_landmark(im_size, landmark_coord, landmark_area, maxVertex=3, maxBrushWidth=24,
                                    area_margin=0.5):
    """
    im_size: tuple, (h, w)
    ...
    if np.random.rand() > 0.5:
        ...
        mask += np_coord_form_mask(random.sample(landmark_coord, k=count), maxBrushWidth=maxBrushWidth, h=im_size[0],
                                   w=im_size[1])
        ...
        mask_area = generate_stroke_mask((area[3] - area[1], area[2] - area[0]), maxVertex=maxVertex,
...
def generate_stroke_mask(
    ...
    if keep_ratio is None:
        ...
        assert isinstance(keep_ratio, (list, tuple)) and keep_ratio[0] >= 0 and keep_ratio[1] <= 1.0 and keep_ratio[0] < \
               keep_ratio[1]
...
def fill_small_holes(mask, fill_value, area_threshold=64, keep_topk=-1, show_contour=False, show_result=False):
    ...
    contours, hierarchy = cv2.findContours(mask_find.copy().astype(np.bool).astype(np.uint8), cv2.RETR_TREE,
