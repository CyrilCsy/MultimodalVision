...
from image_synthesis.data.utils.util import generate_mask_based_on_landmark, generate_stroke_mask, rgba_to_depth, \
    visualize_depth
...
class ImageListDataset(Dataset):
    ...
    def __init__(self,
        ...
        # get image file
        image_relative_paths = self.get_file_list(root=os.path.join(data_root, name), name=name,
                                                  file_list=image_list_file, file_end_with=image_end_with)
        image_paths = [os.path.join(data_root, name, relpath) for relpath in image_relative_paths]

        if load_segmentation_map:
            # import pdb; pdb.set_trace()
            semgmentation_paths = self.get_file_list(root=os.path.join(self.data_root, 'segmentation', name), name=name,
                                                     file_list=image_list_file, file_end_with='.png', path_type='abs')
            ...
            if image_list_file != '':
                semgmentation_paths = [os.path.join(self.data_root, 'segmentation', name, relpath) for relpath in
                                       semgmentation_paths]

            assert len(semgmentation_paths) == len(
                image_paths), 'the number of segmentation maps should be the same with images, image: {}, maps: {}'.format(
                len(image_paths), len(semgmentation_paths))
        else:
            semgmentation_paths = None

        if load_sketch_map:
            sketch_paths = self.get_file_list(root=os.path.join(self.data_root, 'sketch', name), name=self.name,
                                              file_list=image_list_file, file_end_with='.png', path_type='abs')
            if image_list_file != '':
                sketch_paths = [os.path.join(self.data_root, 'sketch', name, relpath) for relpath in sketch_paths]
            assert len(sketch_paths) == len(
                image_paths), 'the number of sketch maps should be the same with images, image: {}, maps: {}'.format(
                len(image_paths), len(sketch_paths))
        else:
            sketch_paths = None

        self.data = ImagePaths(paths=image_paths, sketch_paths=sketch_paths, segmentation_paths=semgmentation_paths,
                               labels={'relative_path': image_relative_paths}, mode=mode)
    ...
    def get_file_list(self, root, name, file_list, file_end_with='.png', path_type='relative'):
        ...
        if os.path.isfile(file_list):
            ...
            if name == 'naturalscene':
                ...
                relpaths = []
                sub_root = ['m/mountain', 'm/mountain_path', 'm/mountain_snowy', 'b/butte', 'c/canyon',
                            'f/field/cultivated', 't/tundra', 'v/valley']
                # count_ = [5000, ]
                for sb in sub_root:
    ...
    def get_erase_mask(self, im_size, erase_mask=False, index=None):
        if self.erase_masks is not None and random.random() < self.use_provided_mask:
            if self.image_mask_paired:
                assert len(self.erase_masks) == len(
                    self.data), 'If image and mask are paired with each other, the number of them should be the same!'
            else:
    ...
    def __getitem__(self, index):
        ...
        if random.random() < self.erase_image_with_mask:
            mask_ = self.get_erase_mask(im_size=(data['image'].shape[1], data['image'].shape[2]),
                                        erase_mask=True)  # H W 1
    ...
    def get_mask_info(self, type='str'):
        if type == 'str':
            if self.use_provided_mask <= 0:
                ...
                info = 'upm{}_mr{}_{}'.format(self.use_provided_mask, self.use_provided_mask_ratio[0],
                                              self.use_provided_mask_ratio[1])
        else:
