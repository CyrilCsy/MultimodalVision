...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, z, token_type=None, topk=1, step=None, total_steps=None):
        ...
        z_q, min_encoding_indices, distance = self._quantize(z_flattened, token_type=token_type_flattened, topk=topk,
...
class PatchVQGAN(BaseCodec):
    ...
    def multi_pixels_with_mask(self, data, mask, mask_pixel_value=None):
        if self.im_process_info['mean'].sum() != 0.0:
            ...
            data_m[~mask] = (
                        (fill_image - self.im_process_info['mean'].to(data_m.device)) / self.im_process_info['std'].to(
                    data_m.device))[~mask]
        else:
            data_m = data * mask.to(data)
            if mask_pixel_value is not None:
                fill_image = torch.zeros_like(data_m) + mask_pixel_value
                data_m[~mask] = ((fill_image - self.im_process_info['mean'].to(data_m.device)) / self.im_process_info[
                    'std'].to(data_m.device))[~mask]
        return data_m
    ...
    def sample(self, batch):
        ...
        if 'erase_mask' in batch:
            token_type_erase = get_token_type(batch['erase_mask'], token_shape, type=self.resize_mask_type)[0].to(
    ...
    def forward(self, batch, name='none', return_loss=True, step=0, total_steps=None, **kwargs):
        ...
        if name == 'generator':
            ...
            if 'erase_mask' in batch:
                token_type_erase, _ = get_token_type(batch['erase_mask'], token_shape,
