...
def get_model_parameters_info(model):
    ...
    for child_name, child_module in model.named_children():
        ...
        parameters[child_name]['total'] = parameters[child_name]['trainable'] + parameters[child_name][
            'non_trainable'] + parameters[child_name]['buffer']
...
def get_all_file(dir, end_with=None, path_type='abs'):
    """
...
def multi_mask_with_image(image, mask, weight=0.25):
    """
    Multiply image with mask

    args:
        image: tensor, array, or PIL.Image
        mask: the same type with image. The 1 (or 255) is unmasked region.
    """
    ...
    im_dark = image * mask
    im = image * weight + im_dark * (1 - weight)
    if is_image:
